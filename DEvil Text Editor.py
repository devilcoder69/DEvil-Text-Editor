...
main_app = tk.Tk()
main_app.geometry('1200x500')
main_app.title('Devil Text Editor')
...
font_size['values'] = tuple(range(8,83,2))
font_size.current(2)
...
def cahnge_font_color():
    color_var = tk.colorchooser.askcolor()
    text_editor.configure(fg=color_var[1])
font_color_btn.configure(command=cahnge_font_color)
...
def find_func(event=None):
    ...
    # button grid
    find_btn.grid(row=2, columnspan=1, padx=8, pady=4)
...
view.add_checkbutton(label='Status Bar',onvalue=1, offvalue=False,variable=show_status_bar, image=status_bar_icon, compound=tk.LEFT, command=hide_statusbar)
